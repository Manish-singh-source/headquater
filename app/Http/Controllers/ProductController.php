        ...
        try {
            ...
            foreach ($reader->getRows() as $record) {
                ...
                $productMapping = ProductMapping::updateOrCreate([
                    ...
                    'portal_code' => $record['Portal Code'] ?? '',
                    'item_code' => $record['Item Code'] ?? '',
                    'basic_rate' => isset($record['Basic Rate']) ? floatval($record['Basic Rate']) : '',
                    'net_landing_rate' => $netLandingRate,
                    'mrp' => $record['MRP'] ?? '',
                ]);
                // }
            ...
            if ($request->has('original_quantity') || $request->has('available_quantity')) {
                ...
                if ($warehouseStock) {
                    ...
                    if ($request->has('available_quantity')) {
                        $updateStockData['original_quantity'] = (int) $request->available_quantity + $warehouseStock->block_quantity;
