@extends('layouts.master')
@section('main-content')

    <!--start main wrapper-->
    <main class="main-wrapper">
        <div class="main-content">
            <div class="row">

                <div class="col-12">
                    <div class="row">
                        <div class="col">
                            <div class="card">
                                <div class="card-header border-bottom-dashed">
                                    <div class="row g-4 align-items-center">
                                        <div class="col-sm">
                                            <h5 class="card-title mb-0">
                                                Raise New Ticket
                                            </h5>
                                        </div>
                                    </div>
                                </div>

                                <div class="card-body">
                                    <div class="row g-3">
                                        <!-- <div class="col-12">
                                            <label for="marital" class="form-label">Issues
                                                <span class="text-danger">*</span></label>
                                            <div>
                                                <span class="me-4">
                                                    <input class="form-check-input" type="checkbox">
                                                    <label for="marital" class="form-label ms-1">Product Missing</label>
                                                </span>
                                                <span class="me-4">
                                                    <input class="form-check-input" type="checkbox">
                                                    <label for="marital" class="form-label ms-1">Product Damage</label>
                                                </span>
                                            </div>
                                        </div> -->
                                        <div class="col-md-12">
                                            <label for="ticket-reason" class="form-label">Ticket Reason</label>
                                            <textarea class="form-control" id="ticket-reason" placeholder="Enter Ticket Reason" rows="3"></textarea>
                                        </div>



                                        <div class="product-table">
                                            <div class="table-responsive white-space-nowrap">
                                                <table class="table align-middle">
                                                    <thead class="table-light">
                                                        <tr>
                                                            <th>Order No</th>
                                                            <th>Portal Code</th>
                                                            <th>SKU Code</th>
                                                            <th>Title</th>
                                                            <th>MRP</th>
                                                            <th>Qty Requirement</th>
                                                            <th>Qty Available</th>
                                                            <th>
                                                                Not Found
                                                            </th>
                                                            <th>
                                                                Damaged
                                                            </th>
                                                            <th>
                                                                Quantity
                                                            </th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>BOCRDVF87G</td>
                                                            <td>TP-260</td>
                                                            <td>Yera 260ml Glass Parabolic Tumbler Set</td>
                                                            <td>315</td>
                                                            <td>64</td>
                                                            <td>40</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>BOCRDL1L94</td>
                                                            <td>JR2KG</td>
                                                            <td>Yera Glass Jar with Plastic Lid - 2425ml</td>
                                                            <td>330</td>
                                                            <td>9</td>
                                                            <td>0</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>BOCRDJH5YZ</td>
                                                            <td>B9OFL</td>
                                                            <td>Yera Ice Cream Delight 250 ml Glass Bowl Set of 6</td>
                                                            <td>280</td>
                                                            <td>64</td>
                                                            <td>50</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>BOCR6N9ZL7</td>
                                                            <td>TC8P17</td>
                                                            <td>Yera Conical Glass Tumbler Set - 215 ml</td>
                                                            <td>230</td>
                                                            <td>144</td>
                                                            <td>100</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>B07T2DJ6JR</td>
                                                            <td>TS10-P0</td>
                                                            <td>Yera Glass Tumbler Transparent 285 ml</td>
                                                            <td>240</td>
                                                            <td>64</td>
                                                            <td>64</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>B07T2D5P2L</td>
                                                            <td>JS-4</td>
                                                            <td>Yera Glass Aahaar Jars, 1800 ml</td>
                                                            <td>190</td>
                                                            <td>144</td>
                                                            <td>14</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>B07T1CN9SX</td>
                                                            <td>T9AHB</td>
                                                            <td>Yera Glass Tumblers - 250 ml, Set of 6</td>
                                                            <td>250</td>
                                                            <td>64</td>
                                                            <td>34</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>B07T1CM6S3</td>
                                                            <td>JR-3</td>
                                                            <td>Yera Glass Aahaar Jars Storage Container, 3600 ML</td>
                                                            <td>225</td>
                                                            <td>360</td>
                                                            <td>200</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>B07T1CM6N6</td>
                                                            <td>CT9-P0</td>
                                                            <td>Yera Transparent Glass Mug with Handle 240 ml</td>
                                                            <td>340</td>
                                                            <td>128</td>
                                                            <td>0</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>OPS/2025/2276</td>
                                                            <td>B07SZ867XZ</td>
                                                            <td>JR-2</td>
                                                            <td>Yera Glass Aahaar Jars Storage Container, 2425 ML</td>
                                                            <td>185</td>
                                                            <td>216</td>
                                                            <td>100</td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-check-input" type="checkbox">
                                                            </td>
                                                            <td>
                                                                <input class="form-input" type="text">
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>

                                        <div class="col-12 col-lg-12 text-end">
                                            <a href="{{route('raise-a-ticket') }}" type="" class="btn btn-success w-sm waves ripple-light text-center mt-2">
                                                Send
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>


                        </div>


                    </div>
                </div>
            </div>
        </div>
    </main>
    <!--end main wrapper-->


       @endsection