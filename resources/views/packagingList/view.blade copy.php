...
@section('main-content')
                                                ...
                                                @php
                                                    ...
                                                    if ($hasAllocations) {
                                                        if ($isAdmin ?? false) {
                                                            ...
                                                            if ($userAllocation) {
                                                                $warehouseName =
                                                                    $userAllocation->warehouse->name ?? 'N/A';
                                                            } else {
    ...
    if ($warehouseStock) {
        if ($isAdmin ?? false) {
            ...
            // Warehouse user: Only show if it's their warehouse
                                                            ...
                                                            $warehouseName =
                                                                $order->warehouseStock->warehouse->name ?? 'N/A';
                                                        } elseif (
                                                            $order->tempOrder &&
                                            ...
                                            <td>
                                                ...
                                                @if ($hasAllocations)
                                                    ...
                                                    @else
                                                        ...
                                                        @php
                                                            $userAllocations = $order->warehouseAllocations->where(
                                                                'warehouse_id',
                                                                $userWarehouseId ?? 0,
                                                            );
                                                        @endphp
                                                        ...
                                                        @else
                                                            ...
                                                            @if ($warehouseStock && $order->tempOrder)
                                                                <div class="mb-1">
                                                                    <strong>{{ $warehouseStock->warehouse->name ?? 'N/A' }}</strong>:
                                                                    {{ $order->tempOrder->block ?? 0 }}
                                                                </div>
                                                            @else
    ...
    $userWarehouseStock = \App\Models\WarehouseStock::where(
        'sku',
        $order->sku,
    )
    ...
    if ($userWarehouseStock) {
        $fallbackWarehouseName =
            $userWarehouseStock->warehouse->name ?? 'N/A';
                                                                }
                                                            }
                                                        @endphp
                                                ...
                                                @php
                                                    ...
                                                    if ($hasAllocations) {
                                                        ...
                                                        if ($isAdmin ?? false) {
                                                            ...
                                                            $totalDispatchQty = $order->warehouseAllocations->sum(
                                                                'allocated_quantity',
                                                            );
                                                        } else {
    ...
    if ($isAdmin ?? false) {
        ...
        // Warehouse user: Show only if it's their warehouse
                                                            if (
                                                                ...
                                                                $order->warehouseStock->warehouse_id ==
                                                                    ($userWarehouseId ?? 0)
                                                            ) {
                                                                $totalDispatchQty = $order->tempOrder->block ?? 0;
                                                            } else {
                                            ...
                                            <td>
                                                ...
                                                @if ($hasAllocations)
                                                    ...
                                                    @if ($isAdmin ?? false)
                                                        ...
                                                        @if ($order->warehouseAllocations->count() > 0)
                                                            @foreach ($order->warehouseAllocations->sortBy('sequence') as $allocation)
                                                                @if ($allocation->final_dispatched_quantity)
                                                                    <div class="mb-1">
                                                                        <strong>{{ $allocation->warehouse->name ?? 'N/A' }}</strong>:
